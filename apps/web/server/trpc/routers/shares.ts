...
export const sharesRouter = router({
  ...
  /* ─── Plan link shares (mirror the recipe procedures) ──────────── */

  activeForPlan: protectedProcedure
    .input(z.object({ planId: z.string().uuid() }))
    .query(({ ctx, input }) =>
      activePlanShareForPlan({ userId: ctx.user.id, planId: input.planId })
    ),
  ...
  createPlan: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        return await createPlanShare({ userId: ctx.user.id, planId: input.planId });
      } catch (error) {
        if (error instanceof FeatureGateDeniedError) {
          throw new TRPCError({
            code: "FORBIDDEN",
        ...
        const message = error instanceof Error ? error.message : "Unknown error.";
        if (message.toLowerCase().includes("not found")) {
          throw new TRPCError({ code: "NOT_FOUND", message, cause: { reason: "NOT_FOUND" } });
        }
        if (message.toLowerCase().includes("only the owner")) {
          throw new TRPCError({ code: "FORBIDDEN", message, cause: { reason: "NOT_CREATOR" } });
        }
        logger.warn("plan_share_create_failed", { userId: ctx.user.id, error: message });
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Couldn't create plan link."
        });
      }
    }),

  revokePlan: protectedProcedure
    .input(z.object({ shareId: z.string().uuid() }))
    .mutation(async ({ ctx, input }) => {
      try {
        await revokePlanShare({ userId: ctx.user.id, shareId: input.shareId });
        return { ok: true as const };
      } catch (error) {
        const message = error instanceof Error ? error.message : "Unknown error.";
        if (message.toLowerCase().includes("not found")) {
