...
import { Camera, CheckCircle2, ChevronDown, Loader2, Plus, Sparkles } from "lucide-react";
...
export function MealLogForm({
  ...
  // Persistent (no auto-clear) note when the AI GENERATED the recipe from just
  // a dish name, so the user knows to review it before saving.
  const [aiDraft, setAiDraft] = React.useState(Boolean(initialSuggestion?.generated));
  ...
  function handleSuggestion(suggestion: MealSuggestion) {
    ...
    setAiDraft(Boolean(suggestion.generated));
      ...
      <div className="rounded-md border border-[var(--border)]">
        ...
        {isRecipeOpen && (
          <div className="grid gap-3 border-t border-[var(--border)] px-3 pb-3 pt-3">
            {aiDraft ? (
              <div className="flex items-start gap-2 rounded-lg border border-[color:var(--warning,#b7791f)]/35 bg-[color:var(--warning,#b7791f)]/10 px-3 py-2 text-[12px] leading-snug text-foreground">
                <Sparkles className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[color:var(--warning,#b7791f)]" />
                <span>
                  This recipe was generated by AI from the dish name. Review and edit it before
                  saving, quantities and steps are a starting point.
                </span>
              </div>
            ) : null}
            <div className="grid gap-[5px]">
