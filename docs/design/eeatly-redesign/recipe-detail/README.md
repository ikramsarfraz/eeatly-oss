...
2. Tick off ingredients as a shopping checklist (with a counter showing how many they still need)
3. Share or copy the shopping list as text
4. Read the recipe broken into numbered steps, with each step showing the ingredients it uses
...
2. `Chip` tone=**ghost**, label "18 ingredients"
3. `Chip` tone=**wheat**, label "3 steps"
...
- Single `Card` (radius 14, border `borderSoft`, padding 0, overflow hidden) containing one row per ingredient. Rows are flex-aligned (center), padding `13px 16px`, separated by 1px `borderSoft` (skip the top border on row 0).
  ...
  - Label: body 14.5/500, color `ink`. When checked, color shifts to `ink3` and gets `line-through` decoration in `ink4`.

The ingredient list for this recipe (in order):

```
Chicken · Salt · Red pepper · Black pepper · Garlic powder · Onion powder ·
Paprika powder · Cumin powder · Ketchup · Sriracha · Chilli garlic sauce ·
Soy sauce · Vinegar · Carrot · Bell pepper · Cabbage · Onions · Tomatoes
```
...
- Column of `Card`s (padding 18, gap 14). Each card:
  - **Header row** — flex, `alignItems: baseline`, gap 12, margin-bottom 12:
    - Italic numeral `1.` / `2.` / `3.` in Instrument Serif italic, **32px**, color `forest` (light) / `forestSoft` (dark). `minWidth: 24`.
    - Step title in Instrument Serif **regular**, 22px, line-height 1.05, color `ink`.
  - **Body** — `flex-wrap` row of ingredient pills used in that step, gap 6:
...
| # | Title | Ingredients |
|---|---|---|
| 1 | Marinate the chicken | Chicken, Salt, Red pepper, Black pepper, Garlic powder, Onion powder, Paprika powder, Cumin powder |
| 2 | Mix the sauce | Ketchup, Sriracha, Chilli garlic sauce, Soy sauce, Vinegar |
| 3 | Prep the vegetables | Carrot, Bell pepper, Cabbage, Onions |
